...
//atoi는 입력받은 문자열을 정수로
//호스트 to network로 변환 포트번호는 시스템에서 넷웤으로 내보내는 것이기 때문에
//hton을 하고 2바이트로 들어가야 해서 s도 붙인다
...
//자식 프로세스를 새로 생성
	...
	if ((pid = fork()) > 0) { // 부모프로세스일때
		input_and_send(accp_sock); //클라는 여기다가 소켓번호를 넣어주고
...
int input_and_send(int sd) {
	...
	while (fgets(buf,sizeof(buf),stdin)!=NULL) {
		...
		if (strstr(buf, EXIT_STRING) != NULL) {
			puts("Good bye");
			close(sd); //인자로 받은 accp_sock을 close
			exit(0);
		}
	}
	return 0;
...
int recv_and_print(int sd) {
	...
	while (1) {
		...
		if (strstr(buf, EXIT_STRING) != NULL) {
						break;
//여기서 break를 썼으므로 exit라는 것을 상대방이 입력해도
//상대방 자신은 끝나도, 그걸 받는 쪽은 그냥 넘긴다.
