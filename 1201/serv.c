...
int main(int argc, char *argv[]) {
	...
	for (count = 0; ; count++) {
		...
		if (accp_sock == -1 && errno != EWOULDBLOCK)
			...
			//clisock_list[num_chat] = accp_sock; 불필요
...
//사용자가 1명이라도 들어와서 accept가 실행되면
//아래 for문도 실행된다. 하지만 당연히 입력값이 없기 때문에
//계속 continue만 걸려서 그냥 ... 만 출력이 되는 것이다.


//그러다가 어느 누군가가 입력을 하게 되면 돌다가 recv해서
//아래 조건문들을 다 통과해서
		for (i = 0; i < num_chat; i++) { 
			...
			buf[nbyte] = 0;
			for (j = 0; j < num_chat; j++) // 여기서 이렇게 다른 클라들한테 전송하는
																//것이다
